...
def main():
    ...
    with open(outdir / f"{Path(pdb).stem}_{tomap}_partlist.out", "w") as out:
        ...
        if args.res:
            ...
            outfile = str(outdir / f"{Path(args.res).stem}_sph_coords.out")  # os.path.splitext(reslist)[0] + "_sph_coords.out"
            parseResidueList(args.res, dPDB, outfile=outfile)
